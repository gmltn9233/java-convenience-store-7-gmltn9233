...
### ConvenienceStore
ConvenienceStore 객체는 StoreInventory와 PromotionManager를 멤버로 가진다.
...
### Order
Order 객체는 물품(Product)과 주문수량(Quantity)를 멤버로 가진다.

### PromotionOrder
PromotionOrder 객체는 Order 클래스를 확장한 클래스로, 주문으로 얻는 증정수량(GiftQuantity)를 추가 멤버로 가진다.

### Orders
Orders 객체는 Order 리스트를 멤버로 가진다.

### Receipt
Receipt 객체는 구매요청상품들(Orders) 과 증정상품들(Orders), 멤버쉽 여부를 멤버로 가진다. 
